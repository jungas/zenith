...
import { niceScale } from '../src/ui/charts.ts';
import { foldToOther } from '../src/views/chart-data.ts';
import type { CategoryDatum } from '../src/ui/charts.ts';
import type { SeriesColor } from '../src/core/model.ts';
...
test('a ninth series folds into Other rather than inventing a hue', () => {
  const rows: CategoryDatum[] = Array.from({ length: 12 }, (_, i) => ({
    label: `Cat ${i}`,
    color: `series-${(i % 8) + 1}` as SeriesColor,
    value: 1_200 - i * 100,
  }));
...
test('a set inside the cap is passed through untouched', () => {
  const rows: CategoryDatum[] = [{ label: 'A', color: 'series-1', value: 10 }];
