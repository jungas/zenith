...
test('a credit card gets exactly one payment envelope, named after it', () => {
  ...
  const payment = paymentEnvelope(state, visa.id);
...
test('positive envelope balances roll forward, negatives do not', () => {
  ...
  const feb = must(buildLedger(state, FEB).get(FEB), `${FEB} in the ledger`);
...
test('pre-existing card debt is not treated as income', () => {
  ...
  const card = creditAccount(state, 'Old Card');
  const payment = paymentEnvelope(state, card.id);
