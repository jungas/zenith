...
import {
  currentMonth, daysBetween, monthOf, nextDayOfMonth, parseISO, todayISO, toISO,
} from './dates.js';
import { accountBalance, categoryRow, monthSummary, ledgerTransactions } from './budget.js';
import { isCredit, paymentCategoryFor } from './model.js';

export const creditAccounts = (state, { includeArchived = false } = {}) =>
  state.accounts.filter((a) => isCredit(a) && (includeArchived || !a.archived));

/** Amount owed on a card, returned positive. */
export function cardBalance(state, cardId, throughISO = null) {
 ...
 */
export function utilizationBand(ratio, warnAt = 0.3) {
 ...
 */
export function statementCycle(card, fromISO = todayISO()) {
...
export function statementBalance(state, card, fromISO = todayISO()) {
 ...
 */
export function cardSnapshot(state, card, { month = currentMonth(), asOf = todayISO() } = {}) {
...
export function cardSnapshots(state, opts = {}) {
  return creditAccounts(state).map((card) => cardSnapshot(state, card, opts));
}

export function monthlyInterest(balanceOwed, apr) {
 ...
 */
export function payoffSchedule(balanceOwed, apr, monthlyPayment, { maxMonths = 600 } = {}) {
  const empty = { months: 0, totalInterest: 0, totalPaid: 0, schedule: [], neverPaysOff: false };
...
export function payoffComparison(card, balanceOwed, monthlyPayment) {
...
export function upcomingPayments(state, { days = 21, asOf = todayISO() } = {}) {
  return cardSnapshots(state)
...
export function debtSummary(state, opts = {}) {
