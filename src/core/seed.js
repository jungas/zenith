...
import { addMonths, currentMonth, daysInMonth, todayISO } from './dates.js';
import { emptyState, ensurePaymentCategories, makeAccount, makeCategory, makeTransaction } from './model.js';
 ...
 */
export function seedState({ months = 4, now = new Date() } = {}) {
  ...
  const between = ([min, max]) => (min === max ? min : min + Math.round(random() * (max - min)));
  ...
  for (const month of monthKeys) {
    ...
    for (const spec of CATEGORIES) {
      const category = byName.get(spec.name);
  ...
  for (const month of monthKeys) {
    const row = {};
    for (const spec of CATEGORIES) row[byName.get(spec.name).id] = spec.budget;
    budgets[month] = row;
  }
  ...
  for (let i = 1; i < monthKeys.length; i++) {
    ...
    for (const card of [visa, mastercard]) {
  ...
  const emergency = byName.get('Emergency fund');
