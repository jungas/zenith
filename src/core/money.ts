...
import type { Cents, MoneyOptions } from './model.ts';
...
function formatter(locale: string, currency: string, options: Intl.NumberFormatOptions): Intl.NumberFormat {
...
export function formatMoney(cents: Cents, opts: MoneyOptions = {}): string {
  const {
    currency = 'USD',
    locale = 'en-US',
    signed = false,
...
export function formatMoneyCompact(cents: Cents, opts: MoneyOptions = {}): string {
