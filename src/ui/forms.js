...
import { openModal, close as closeModal, confirmDialog } from './modal.js';
import { toast } from './toast.js';
import { field, input, moneyInput, select, segmented, button, statusPill } from './components.js';
import { icon } from './icons.js';
import { parseMoney, centsToInput, formatMoney } from '../core/money.js';
import { todayISO } from '../core/dates.js';
import { ACCOUNT_TYPES, CATEGORY_COLORS, isCredit, paymentCategoryFor } from '../core/model.js';
import { cardSnapshot } from '../core/cards.js';
import { commit, getState, moneyOpts, undo } from '../store.js';
import * as actions from '../core/actions.js';

const money = () => moneyOpts();

function accountOptions(state, { selected, filter = () => true } = {}) {
  return state.accounts
    .filter((a) => !a.archived && filter(a))
    .map((a) => ({ value: a.id, label: a.name, selected: a.id === selected }));
}

function categoryOptions(state, { selected, includePayments = false } = {}) {
  ...
  for (const category of state.categories) {
    ...
    groups.get(category.group).push({
      value: category.id,
      label: category.name,
...
/**
 * @param {{transaction?: object, defaults?: object}} [opts]
 */
export function openTransactionForm({ transaction = null, defaults = {} } = {}) {
  ...
  const body = h('form.form', { onsubmit: (event) => event.preventDefault() });
  ...
  const clearedInput = h('input', { type: 'checkbox', class: 'checkbox', checked: transaction?.cleared ?? false });
  ...
  const renderHint = () => {
    ...
    if (!account || !isCredit(account)) return mount(hintSlot);
    return mount(
      hintSlot,
      h(
        'div.inline-note',
...
export function openPaymentForm(cardId) {
  const state = getState();
  ...
  const dateInput = h('input.input', { type: 'date', value: todayISO(), required: true });
...
function suggestedPayment(snapshot) {
...
export function openAccountForm({ account = null, presetType } = {}) {
  const editing = Boolean(account);
  let type = account?.type ?? presetType ?? 'checking';
  ...
  const render = () => {
    ...
    const limitInput = moneyInput({ value: account?.creditLimit ? centsToInput(account.creditLimit) : '' });
    const aprInput = h('input.input', {
      type: 'number', step: '0.01', min: '0', max: '99',
      value: account?.apr ? (account.apr * 100).toFixed(2) : '',
      placeholder: '19.99',
    });
    const statementInput = h('input.input', {
      type: 'number', min: '1', max: '31', value: account?.statementDay ?? 18,
    });
    const dueInput = h('input.input', { type: 'number', min: '1', max: '31', value: account?.dueDay ?? 12 });
    const minRateInput = h('input.input', {
      type: 'number', step: '0.1', min: '0', max: '100',
      value: ((account?.minPaymentRate ?? 0.02) * 100).toFixed(1),
    });
    const minFloorInput = moneyInput({ value: centsToInput(account?.minPaymentFloor ?? 2500) });
    ...
    editing
      ? h('button.btn.btn-danger-ghost', {
          ...
          onclick: async () => {
            ...
            if (!ok) return;
            commit((s) => actions.deleteAccount(s, account.id), { label: 'delete account' });
            closeModal();
  ...
  render();
  openModal({ title: editing ? `Edit ${account.name}` : 'Add account', body, footer });
}
...
export function openCategoryForm({ category = null } = {}) {
  ...
  let color = category?.color ?? CATEGORY_COLORS[state.categories.length % CATEGORY_COLORS.length];
  ...
  const submit = h('button.btn.btn-primary', {
    ...
    onclick: () => {
      ...
      if (editing) commit((s) => actions.updateCategory(s, category.id, patch), { label: 'edit category' });
      else commit((s) => actions.addCategory(s, patch), { label: 'add category' });
      closeModal();
      undoToast(editing ? 'Category updated.' : 'Category added.');
    },
  });
  ...
  openModal({ title: editing ? `Edit ${category.name}` : 'New category', body, footer, size: 'sm' });
}

/* ── Move money between envelopes ─────────────────────────────────────── */

export function openMoveMoneyForm({ month, fromCategoryId, available }) {
  const state = getState();
 ...
 */
export function openFundCardForm(cardId, { month }) {
  const state = getState();
  const card = state.accounts.find((a) => a.id === cardId);
  const paymentCategory = paymentCategoryFor(state, cardId);
  if (!card || !paymentCategory) return;
