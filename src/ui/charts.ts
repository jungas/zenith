...
import { h, append, mount } from './dom.ts';
import { icon } from './icons.ts';
import { formatMoneyCompact, formatMoney } from '../core/money.ts';
import type { Cents, MoneyOptions, SeriesColor } from '../core/model.ts';
...
/** One period in a multi-series chart; `values` is parallel to `series`. */
...
/** A row in a tooltip: either a bare label or a labelled value with a swatch. */
...
type MeterStatus = 'accent' | 'good' | 'warning' | 'serious' | 'critical' | 'neutral';
 ...
 */
export function niceScale(value: number, targetTicks = 5): { max: number; ticks: number[] } {
...
function barPathHorizontal(x: number, y: number, width: number, height: number): string {
...
function barPathVertical(x: number, baseline: number, width: number, height: number): string {
...
function withTooltip(figure: HTMLElement, svg: SVGSVGElement) {
  const tip: HTMLElement = h('div.chart-tip', { role: 'status', 'aria-live': 'polite' });
  figure.appendChild(tip);

  const show = (event: { clientX: number; clientY: number }, lines: [string, ...TooltipRow[]]) => {
    const [title, ...rows] = lines;
    mount(
      tip,
      h('div.chart-tip-title', { text: title }),
      ...rows.map((line) =>
        h(
          'div.chart-tip-row',
          null,
          line.color ? h('span.chart-tip-swatch', { style: { background: `var(--${line.color})` } }) : null,
          h('span.chart-tip-label', { text: line.label }),
...
export function legend(items: SeriesSpec[]): HTMLElement {
  return h(
    'ul.legend',
    { role: 'list' },
    items.map((item) =>
...
export function tableView(
  headers: string[],
  rows: string[][],
  { summary = 'Show data table' }: { summary?: string } = {},
): HTMLElement {
  return h(
    'details.table-view',
    null,
...
function emptyChart(title: string | undefined, subtitle: string | undefined, message: string): HTMLElement {
...
export function categoryBars(
  rows: CategoryDatum[],
  {
    title, subtitle, money = {}, max: forcedMax,
    emptyMessage = 'No spending recorded yet.',
  }: {
    title?: string; subtitle?: string; money?: MoneyOptions;
    max?: number; emptyMessage?: string;
  } = {},
): HTMLElement {
  ...
  const group = h<SVGGElement>('g', { class: 'bar-row', tabindex: '0', role: 'listitem' });
...
export function groupedColumns(
  periods: PeriodDatum[],
  series: SeriesSpec[],
  { title, subtitle, money = {} }: { title?: string; subtitle?: string; money?: MoneyOptions } = {},
): HTMLElement {
  ...
  periods.forEach((period, index) => {
    ...
    const group = h<SVGGElement>('g', { class: 'column-group', tabindex: '0' });
...
export function lineChart(
  points: LinePoint[],
  {
    title, subtitle, money = {}, color = 'series-1', valueLabel = 'Balance',
  }: {
    title?: string; subtitle?: string; money?: MoneyOptions;
    color?: SeriesColor; valueLabel?: string;
  } = {},
): HTMLElement {
  ...
  const px = (i: number): number => pad.left + i * stepX;
  const py = (value: number): number => baseline - (value / max) * plotHeight;
  ...
  const crosshair = h<SVGLineElement>('line', { class: 'crosshair', y1: pad.top, y2: baseline, x1: 0, x2: 0, opacity: '0' });
  ...
  svg.addEventListener('pointermove', (event: PointerEvent) => {
    ...
    crosshair.setAttribute('x1', String(px(index)));
    crosshair.setAttribute('x2', String(px(index)));
 ...
 */
export function meter({ ratio, status = 'accent', label, caption, ariaLabel }: MeterOptions): HTMLElement {
...
export function sparkline(
  values: number[],
  {
    color = 'series-1', width = 96, height = 28,
  }: { color?: SeriesColor; width?: number; height?: number } = {},
