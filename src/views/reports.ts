...
import { statTile, sectionHeader, emptyState, segmented } from '../ui/components.ts';
import { categoryBars, groupedColumns, lineChart, tableView } from '../ui/charts.ts';
import { formatMoney, formatPercent } from '../core/money.ts';
import { addMonths, currentMonth, monthLabel, monthLabelShort, monthRange } from '../core/dates.ts';
import { ledgerMonths, monthlyTotals, spendingByCategory, totalDebt } from '../core/budget.ts';
import { getState, moneyOpts } from '../store.ts';
import { foldToOther, toCategoryRows } from './chart-data.ts';
import type { MoneyOptions } from '../core/model.ts';
import type { LinePoint } from '../ui/charts.ts';
...
export function reportsView(): HTMLElement {
...
function debtChangeHint(points: LinePoint[], money: Required<Pick<MoneyOptions, 'currency' | 'locale'>>): string | undefined {
