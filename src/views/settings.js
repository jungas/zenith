      ...
      h(
        ...
        field(
          ...
          input({
            ...
            onchange: (event) => {
              const pct = Math.min(100, Math.max(5, Number(event.target.value) || 30));
  ...
  const fileInput = h('input', {
    ...
    onchange: async (event) => {
      ...
      try {
        ...
        toast(error.message || 'That file could not be read.', { tone: 'error' });
      } finally {
        event.target.value = '';
      }
    },
...
function integrityRow(label, cents, money, emphasis = false) {
  return h(
    'li',
