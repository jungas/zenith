...
import { h, append, mount, debounce } from '../ui/dom.ts';
import { icon } from '../ui/icons.ts';
import { sectionHeader, emptyState, moneyText, select, input } from '../ui/components.ts';
import { openTransactionForm } from '../ui/forms.ts';
import { currentMonth, formatDateShort, monthLabel } from '../core/dates.ts';
import { ledgerMonths, queryTransactions } from '../core/budget.ts';
import { isCredit } from '../core/model.ts';
import { getState, moneyOpts } from '../store.ts';
import { setQuery } from '../router.ts';
import type { RouteParams } from '../router.ts';
import type { AppState, MoneyOptions, Transaction } from '../core/model.ts';
import type { TransactionFilters } from '../core/budget.ts';

export function transactionsView(params: RouteParams = {}): HTMLElement {
  ...
  const accountSelect = select(
    ...
    ],
    {
      ...
      onchange: (event: Event) => {
        filters.accountId = (event.target as HTMLSelectElement).value;
  ...
  const categorySelect = select(
    ...
    ],
    {
      ...
      onchange: (event: Event) => {
        filters.categoryId = (event.target as HTMLSelectElement).value;
  ...
  const monthSelect = select(
    ...
    ],
    {
      ...
      onchange: (event: Event) => {
        filters.month = (event.target as HTMLSelectElement).value;
  ...
  function renderList(): void {
    ...
    const inflow = rows.filter((r) => r.amount > 0).reduce((t: number, r) => t + r.amount, 0);
    const outflow = rows.filter((r) => r.amount < 0).reduce((t: number, r) => t + r.amount, 0);
...
function groupByDate(rows: Transaction[], state: AppState, money: Required<Pick<MoneyOptions, 'currency' | 'locale'>>): HTMLElement[] {
...
export function transactionRow(tx: Transaction, state: AppState, money: Required<Pick<MoneyOptions, 'currency' | 'locale'>>): HTMLElement {
